...
import { 
  Home, ShoppingCart, MessageSquare, Wallet, Users, 
  List, Youtube, FileText, DollarSign, Settings, Plus 
} from 'lucide-react';
...
const NavItem: React.FC<NavItemProps> = ({ icon: Icon, label, to }) => (
  <Link 
    to={to} 
    className="flex items-center p-3 rounded-md text-sidebar-foreground hover:bg-sidebar-accent hover:text-sidebar-accent-foreground transition-colors duration-200"
  >
  ...
  return (
    <aside className="bg-sidebar text-sidebar-foreground flex flex-col h-full w-full border-r border-sidebar-border dark:border-sidebar-border">
      ...
      <nav className="flex-1 p-4 space-y-2 overflow-y-auto">
        ...
        <div className="text-xs font-semibold text-sidebar-foreground/70 uppercase mt-4 pt-4 border-t border-sidebar-border dark:border-sidebar-border">
          Services
        </div>
        ...
        <NavItem icon={Youtube} label="Youtube Downloader" to="/youtube-downloader" />
        ...
        <div className="text-xs font-semibold text-sidebar-foreground/70 uppercase mt-4 pt-4 border-t border-border dark:border-sidebar-border">
          Affiliate
        </div>
